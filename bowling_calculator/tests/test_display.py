...
def test_unpack_tuple():
    test_player = Player(
        "John", ["12", "45", "5/", "8-", "9/", "X", "X", "X", "X", "X"], 56
    )
    assert unpack_tuple(test_player) == (
        "John",
...
def test_display_leaderboard():
    lane = "lane1"
    players = [
        Player(name="Jane", scores="12|45|5/|8-|9/|X|X|X|X|X", current_result=148),
